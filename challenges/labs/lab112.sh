...
while true; do
  ...
  # STEP 4: Inspect I/O ports (legacy/PCI BAR I/O windows)
  echo "  Step 4: Show the first 15 lines of /proc/ioports."
  read -p "  lab@lpic-lab112:~$ " cmd4
  echo
  if [[ "$cmd4" != "head -n 15 /proc/ioports" && "$cmd4" != "cat /proc/ioports | head -n 15" ]]; then
  ...
  # STEP 5: Inspect physical memory map
  echo "  Step 5: Show 'System RAM' regions from /proc/iomem."
  read -p "  lab@lpic-lab112:~$ " cmd5
  echo
  if [[ "$cmd5" != "grep -i 'System RAM' /proc/iomem | head -n 5" && \
        "$cmd5" != "cat /proc/iomem | grep -i 'System RAM' | head -n 5" ]]; then
  ...
  # STEP 6: Correlate PCI device -> driver -> module
  echo "  Step 6: From lspci, print the Ethernet device and show its driver section."
  read -p "  lab@lpic-lab112:~$ " cmd6
  echo
  if [[ "$cmd6" != "lspci -nnk | grep -A2 -i ethernet" && "$cmd6" != "sudo lspci -nnk | grep -A2 -i ethernet" ]]; then
  ...
  # STEP 7: Verify the module for that driver is loaded
  echo "  Step 7: Confirm that the e1000 kernel module is currently loaded."
  read -p "  lab@lpic-lab112:~$ " cmd7
  echo
  if [[ "$cmd7" != "lsmod | grep -E '^e1000(\\s|$)'" && "$cmd7" != "lsmod | grep ^e1000" && "$cmd7" != "grep e1000 /proc/modules" ]]; then
  ...
  print_info "surveyed legacy I/O ports in /proc/ioports, inspected physical memory regions"
  print_info "in /proc/iomem, and correlated a PCI device to its driver and module."
