...
# Lab 239: Configure a Static IP with nmcli (server1) — SIMULATED & SAFE
# SAFETY: Validates typed commands and prints canned outputs only. No real network changes occur.
# Output policy: Only show realistic, canned command output. Silent commands produce no output.
...
LAB_NAME="Lab 239: nmcli static IP (server1)"
LAB_ID="lab239"
LAB_XP=20500
...
CON="server1"
IFACE="eth0"
IP4_CIDR="192.168.50.10/24"
GW4="192.168.50.1"
DNS="1.1.1.1 8.8.8.8"
UUID="a1b2c3d4-1111-2222-3333-abcdefabcdef"
...
while true; do
  ...
  # Step 2: Create the connection with static IP & gateway (accept two common syntaxes)
  echo "  Step 2: Create '${CON}' on ${IFACE} with ${IP4_CIDR} and gateway ${GW4}."
  read -p "  lab@lab239:~$ " cmd2
  ...
  echo "  Step 3: Add DNS servers '${DNS}' to '${CON}' (also ensure autoconnect)."
  read -p "  lab@lab239:~$ " cmd3a
  [[ "$cmd3a" != "nmcli con mod ${CON} ipv4.dns \"${DNS}\"" ]] && {
    print_error "Hint: nmcli con mod ${CON} ipv4.dns \"${DNS}\""
    read -p "Press Enter to try again..." _
    continue
  }
  read -p "  lab@lab239:~$ " cmd3b
  ...
  read -p "  lab@lab239:~$ " cmd6
  if [[ "$cmd6" == "ip -4 addr show dev ${IFACE}" ]]; then
    echo "  2: ${IFACE}: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc fq_codel state UP group default qlen 1000"
    echo "      link/ether 52:54:00:ab:cd:ef brd ff:ff:ff:ff:ff:ff"
    echo "      inet ${IP4_CIDR} brd 192.168.50.255 scope global noprefixroute ${IFACE}"
  ...
  read -p "  lab@lab239:~$ " cmd7
  if [[ "$cmd7" == "ping -c 2 ${GW4}" ]]; then
    echo "  PING ${GW4} (${GW4}) 56(84) bytes of data."
    echo "  64 bytes from ${GW4}: icmp_seq=1 ttl=64 time=0.492 ms"
    echo "  64 bytes from ${GW4}: icmp_seq=2 ttl=64 time=0.503 ms"
    echo
    echo "  --- ${GW4} ping statistics ---"
    echo "  2 packets transmitted, 2 received, 0% packet loss, time 1002ms"
    echo "  rtt min/avg/max/mdev = 0.492/0.498/0.503/0.006 ms"
  else
